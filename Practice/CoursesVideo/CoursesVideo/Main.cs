...
namespace CoursesVideo
{
    public partial class Main : Form
    {
        ...
        private void Load_Child(int idParent)
        {
            ...
            {
                connection.Open();
                string query = "select * from courses where instructor_id = @id";
                SqlDataAdapter adapter = new(query, connection);
        ...
        private void Modify_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                string query = "update courses set nume = @name, durata = @durata where id = @id";
        ...
        private void Save_Click(object sender, EventArgs e)
        {
            ...
            {
                connection.Open();
                string query = "insert into courses (nume, durata, instructor_id) values (@nume,@durata,@instructor_id)";
                SqlCommand command = new(query, connection);
                command.Parameters.AddWithValue("@nume", textBox1.Text);
                command.Parameters.AddWithValue("@durata", TimeSpan.Parse(maskedTextBox1.Text));
                command.Parameters.AddWithValue("@instructor_id", Convert.ToInt32(textBox2.Text));
                command.ExecuteNonQuery();
            }
            catch (SqlException ex)
            {
