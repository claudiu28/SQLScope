...
namespace Streaming
{
    public partial class Main : Form
    {
        ...
        private void Load_Child(int idParent)
        {
            ...
            {
                connection.Open();
                string query = "select * from filme where actor_id = @id";
                SqlDataAdapter adapter = new(query, connection);
        ...
        private void Modify_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                string query = "update filme set nume = @name, durata = @durata, website = @website where id = @id";
                SqlCommand command = new(query, connection);
                command.Parameters.AddWithValue("@name", textBox1.Text);
                command.Parameters.AddWithValue("@durata",TimeSpan.Parse(maskedTextBox1.Text));
                command.Parameters.AddWithValue("@website", textBox3.Text);
        ...
        private void Save_Click(object sender, EventArgs e)
        {
            ...
            {
                connection.Open();
                string query = "insert into filme (nume, durata, website, actor_id) values (@nume,@durata,@website,@actor_id)";
                SqlCommand command = new(query, connection);
                command.Parameters.AddWithValue("@nume", textBox1.Text);
                command.Parameters.AddWithValue("@durata", TimeSpan.Parse(maskedTextBox1.Text));
                command.Parameters.AddWithValue("@website", textBox3.Text);
                command.Parameters.AddWithValue("@actor_id", Convert.ToInt32(textBox2.Text));
                command.ExecuteNonQuery();
            }
            catch (SqlException ex)
            {
