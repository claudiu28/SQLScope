...
namespace companies
{
    public partial class Main : Form
    {
        ...
        private void Load_Child(int idParent)
        {
            ...
            {
                connection.Open();
                string query = "select * from employees where companies_id = @id";
                SqlDataAdapter adapter = new(query, connection);
        ...
        private void Modify_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                string query = "update employees set nume = @name, salary = @salary where id = @id";
                SqlCommand command = new(query, connection);
                command.Parameters.AddWithValue("@name", textBox1.Text);
                command.Parameters.AddWithValue("@salary", Convert.ToInt32(numericUpDown1.Value));
        ...
        private void Save_Click(object sender, EventArgs e)
        {
            ...
            {
                connection.Open();
                string query = "insert into employees(nume, salary, companies_id) values (@nume,@salary,@companies_id)";
                SqlCommand command = new(query, connection);
                command.Parameters.AddWithValue("@nume", textBox1.Text);
                command.Parameters.AddWithValue("@salary", Convert.ToInt32(numericUpDown1.Value));
                command.Parameters.AddWithValue("@companies_id", Convert.ToInt32(textBox2.Text));
                command.ExecuteNonQuery();
            }
            catch (SqlException ex)
            {
